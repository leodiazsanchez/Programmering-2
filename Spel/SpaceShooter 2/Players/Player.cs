        ...
        public void Update(GameWindow window, GameTime gameTime, ContentManager content)
        {
            ...
            switch (who)
            {
                ...
                case 2:
                    ...
                    numFrames = 5;
                    spritePath = content.Load<Texture2D>("images/player2/idle");
                    isAttacking = false;

                    if (keyboardState.IsKeyDown(Keys.Right))
                    {
                        speed.X += 1.5f;
                        spritePath = content.Load<Texture2D>("images/player2/run"); ;
                        numFrames = 6;
                        rotation = SpriteEffects.None;
                    }
                    ...
                    {
                        speed.X = 0;
                        spritePath = content.Load<Texture2D>("images/player2/attack");
                        numFrames = 5;
                        if (gameTime.TotalGameTime.TotalMilliseconds > timeSinceLastBullet + 800)
                        {
                            isAttacking = true;
            ...
            vector.Y = Math.Min(Math.Max(vector.Y, 0), window.ClientBounds.Height - texture.Height - 120);
